...
function createArmyMovement(from: GalaxyPlanet, to: GalaxyPlanet) {
    return { from: from.id, to: to.id, armyCount: 1 }; // from.armyCount * .5};
}

function determineRandomArmyMovement(faction: number, planets: GalaxyPlanet[]): ArmyMovement | undefined {
...
export function determineNextArmyMovementAction(method: AiMethod, faction: number, planets: GalaxyPlanet[]): ArmyMovement | undefined {
