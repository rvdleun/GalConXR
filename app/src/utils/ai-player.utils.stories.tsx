...
import { AiMethod, determineNextArmyMovementAction } from "./ai-player.utils.ts";
...
const render = ({ planets }: { planets: GalaxyPlanet[] }) => {
    ...
    const runAiTick = () => {
        setResult(JSON.stringify(determineNextArmyMovementAction(AiMethod.RANDOM, 1, planets)));
    }
    ...
    return <>
        {result && <input style={{ position: 'fixed', top: 0, left: 0, width: '100vw', zIndex: 1 }} value={result} /> }
        <button onClick={runAiTick} style={{ position: 'fixed', bottom: '10px', left:'10px', zIndex: 1 }}>Run AI Tick</button>
