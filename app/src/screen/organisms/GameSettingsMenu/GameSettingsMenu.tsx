...
export const GameSettingsMenu: FC<GameSettingsMenuProps> = ({onNewSettings, settings, x, y, width}) => {
    ...
    return <>
        <Select
            ...
            selectedOption={galaxySizeOptions.find(option => option.value === settings.galaxySize)!}
             x={y} y={y} width={width} />
        <Select
            onNewSelectedOption={handleUpdateNumberOfPlayers}
            options={numberOfPlayersOptions}
            selectedOption={numberOfPlayersOptions.find(option => option.value === settings.numberOfPlayers)!}
             x={y} y={y + 100} width={width} />
    </>
} 